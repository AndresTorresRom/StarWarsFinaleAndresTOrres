...
export class PeliculasComponent implements OnInit {
  ...
  //Método para buscar la pelicula solicitada en la barra de búsqueda 
  onFindMovie(name: string){
    let index: number = this.names.indexOf(name); //Busca el indice de la pelicula solicitada en el arreglo de nombres 'names'
    if(index === -1){ //Si el nombre de la pelicula no esta en en el arreglo de nombres, mostrará una alerta 
      alert("Starship not found...");
    } else {
      this.getMovie(index); //Crea la pelicula que se mostrara en la vista 
    }
  }
}
